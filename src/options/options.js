...
const API = globalThis.SIGNSAFE_SHARED?.constants?.API || {};
...
const PRIMARY_PING_ENDPOINT = API.PING_ENDPOINT || "https://api.signsafe.xyz/v1/ping";
const FALLBACK_PING_ENDPOINT = API.FALLBACK_PING_ENDPOINT || "https://signsafe-api.utkuvonarslan.workers.dev/v1/ping";
...
async function testConnection() {
  ...
  try {
    const res = await pingWithFallback(key);
...
async function pingWithFallback(key) {
  try {
    const primary = await pingEndpoint(PRIMARY_PING_ENDPOINT, key);
    if (primary.status >= 500 && FALLBACK_PING_ENDPOINT) {
      return await pingEndpoint(FALLBACK_PING_ENDPOINT, key);
    }
    return primary;
  } catch (_error) {
    if (!FALLBACK_PING_ENDPOINT) {
      throw _error;
    }
    return await pingEndpoint(FALLBACK_PING_ENDPOINT, key);
  }
